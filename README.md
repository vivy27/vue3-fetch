...
If you'd rather not use the wrapper component (e.g. you don't need the template slot, or you want to compose fetch state with other logic), use the `useFetch` composable directly. It accepts the same options as the component's props and returns the same reactive state.
